...
REGULAREXPRESSION(URLRegularExpression,@"(http[s]{0,1}://[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z]{2,4})(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)|(www.[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z]{2,4})(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)")
//@"(?i)(?:(?:https?):\\/\\/)?(?:\\S+(?::\\S*)?@)?(?:(?:[1-9]\\d?|1\\d\\d|2[01]\\d|22[0-3])(?:\\.(?:1?\\d{1,2}|2[0-4]\\d|25[0-5])){2}(?:\\.(?:[1-9]\\d?|1\\d\\d|2[0-4]\\d|25[0-4]))|(?:(?:[a-z\\u00a1-\\uffff0-9]+-?)*[a-z\\u00a1-\\uffff0-9]+)(?:\\.(?:[a-z\\u00a1-\\uffff0-9]+-?)*[a-z\\u00a1-\\uffff0-9]+)*(?:\\.(?:[a-z\\u00a1-\\uffff]{2,})))(?::\\d{2,5})?(?:\\/[^\\s]*)?")
REGULAREXPRESSION(PhoneNumerRegularExpression,@"(1[- ]?(3[- ]?[4-9][- ]?|5[- ]?[0127-9][- ]?|8[- ]?[23478][- ]?|4[- ]?7[- ]?|7[- ]?8[- ]?|7[- ]?0[- ]?)(([0-9][- ]?){4}|5([0-9][- ]?){3})([0-9][- ]?){4})|(1[- ]?(3[- ]?[0-2][- ]?|5[- ]?[56][- ]?|8[- ]?[56][- ]?|7[- ]?6[- ]?|4[- ]?5[- ]?|7[- ]?0[- ]?)(([0-9][- ]?){4}|9[- ]?([0-9][- ]?){3})([0-9][- ]?){4})|(1[- ]?(3[- ]?3[- ]?|5[- ]?3[- ]?|8[- ]?[019][- ]?|7[- ]?7[- ]?|7[- ]?0[- ]?)(([0-9][- ]?){4}|0[- ]?([0-9][- ]?){3})([0-9][- ]?){4})" )//
...
//@"(1(3[4-9]|5[0127-9]|8[23478]|47|78|70)[- ]?([0-9]{4}|5[0-9]{3})[- ]?[0-9]{4})|(1(3[0-2]|5[56]|8[56]|76|45|70)[- ]?([0-9]{4}|9[0-9]{3})[- ]?[0-9]{4})|(1(33|53|8[019]|77|70)[- ]?([0-9]{4}|0[0-9]{3})[- ]?[0-9]{4})"
//@"\\d{3}-\\d{8}|\\d{3}-\\d{7}|\\d{4}-\\d{8}|\\d{4}-\\d{7}|1+[358]+\\d{9}|\\d{8}|\\d{7}"
@interface MLLink()
...
- (void)commonInit
{
    ...
    //默认除了话题和@都检测
    _dataDetectorTypes = MLDataDetectorTypeURL|MLDataDetectorTypePhoneNumber|MLDataDetectorTypeEmail|MLDataDetectorTypeAttributedLink|MLDataDetectorTypeUserHandle;
...
- (NSMutableArray*)linksWithString:(id)string
{
    ...
    for (NSRegularExpression *regexp in regexps) {
        ...
        if ([regexps indexOfObject:regexp] == 0) { //第一个是URL URL检测用新方法做，不使用正则表达式
            NSDataDetector *URLDetector = [NSDataDetector dataDetectorWithTypes:NSTextCheckingTypeLink error:nil];
            NSArray *matches = [URLDetector matchesInString:plainText options:0 range:textRange];
            //fix range
            for (NSTextCheckingResult *result in matches) {
                NSRange URLRange = [self fixRangeForURL:result.range withContent:plainText];
                
                BOOL containLink = NO;
                //去重处理
                for (MLLink *link in links){
                    if (NSMaxRange(NSIntersectionRange(link.linkRange, URLRange))>0){
                        containLink = YES;
                        break;
                    }
                }
                ...
                //这个刚好和MLLinkType对应
                MLLinkType linkType = MLLinkTypeURL;
                
                if (linkType != MLLinkTypeNone) {
                    MLLink *link = [MLLink linkWithType:linkType value:[plainText substringWithRange:URLRange] range:URLRange];
                    if (self.beforeAddLinkBlock) {
                        self.beforeAddLinkBlock(link);
                    }
                    [links addObject:link];
        ...
        {
            [regexp enumerateMatchesInString:plainText options:0 range:textRange
...
- (NSRange)fixRangeForURL:(NSRange)oldRange withContent:(NSString*)string
{
    NSUInteger end = oldRange.location + oldRange.length;
    for (NSUInteger i = oldRange.location; i < end; i++) {
        unichar char_text = [string characterAtIndex:i];
        if (char_text <=32 || char_text >= 126) {
            return NSMakeRange(oldRange.location, i - oldRange.location);
        }
    }
    return oldRange;
}
