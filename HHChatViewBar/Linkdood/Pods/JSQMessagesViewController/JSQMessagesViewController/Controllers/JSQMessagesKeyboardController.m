...

{
    ...
    if (self) {
        ...
        [panGestureRecognizer addTarget:self action:@selector(jsq_handlePanGestureRecognizer:)];
        [tapGestureRecognizer addTarget:self action:@selector(jsq_handleTapGestureRecognizer:)];
...
//        [self.tapGestureRecognizer addTarget:self action:@selector(jsq_handleTapGestureRecognizer:)];
    }];
}
...
//        [self.tapGestureRecognizer removeTarget:self action:NULL];
    }];
}
...
- (void)jsq_resetKeyboardAndTextView
{
    if (self.textView.isFirstResponder) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(resetInputModule)]) {
            [self.delegate resetInputModule];
        }
    }
}
...
- (void)jsq_handleTapGestureRecognizer:(UITapGestureRecognizer *)pan
{
    [self jsq_resetKeyboardAndTextView];
}
