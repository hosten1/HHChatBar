...
@property (weak,nonatomic) IBOutlet PBSaveBtn *scanBtn;
...
-(void)zoomPhotoVC{
    ...
    [_handleVC setNeedsStatusBarAppearanceUpdate];
}
...
-(void)showWithPage:(NSUInteger)page{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        self.qrCode = [LCQRCodeUtil readQRCodeFromImage:photoItemView.photoModel.image];
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.qrCode == nil) {//如果二维码存在则显示二维码识别按钮
...
- (IBAction)leftBtnClick:(id)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(scanQRCode:)]) {
        [self.delegate scanQRCode:self.qrCode];
        [self dismiss];
    }
}
...
- (IBAction)rightBtnClick:(id)sender {
    ...
    if(!itemView.hasImage){
        [SVProgressHUD showInfoWithStatus:@"无图片数据" maskType:SVProgressHUDMaskTypeClear];
        return;
    }
    
    //读取缓存
    if([itemModel read]){//已经保存过本地
        [SVProgressHUD showInfoWithStatus:@"图片已存" maskType:SVProgressHUDMaskTypeBlack];
    }else{
        ...
        [itemView save:^{
            ...
            [SVProgressHUD showErrorWithStatus:@"保存失败" maskType:SVProgressHUDMaskTypeClear];
        }];
    }
}
