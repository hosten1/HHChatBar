...
- (UICollectionViewCell *)collectionView:(JSQMessagesCollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.showMessageSelector) {
        UIImageView *selector = [[UIImageView alloc]initWithImage:[self collectionView:collectionView selectorImageDataForItemAtIndexPath:indexPath]];
        [selector setTag:1001];
        if (!isOutgoingMessage) {
            JSQMessagesCollectionViewCellIncoming *incomingCell = (JSQMessagesCollectionViewCellIncoming *)cell;
            incomingCell.selectorWidthContraint.constant = 40.0;
            [selector setCenter:CGPointMake(20, cell.frame.size.height/2)];
            [incomingCell.selectorContainView addSubview:selector];
        }else{
            [selector setCenter:CGPointMake(20, cell.frame.size.height/2)];
        ...
        if (!isOutgoingMessage) {
            JSQMessagesCollectionViewCellIncoming *incomingCell = (JSQMessagesCollectionViewCellIncoming *)cell;
            incomingCell.selectorWidthContraint.constant = 0;
        }
    }
...
//    }
    ...
    CGFloat bubbleTopLabelInset = 60.0f;//(avatarImageDataSource != nil) ? 60.0f : 15.0f;
...
- (void)messagesInputToolbar:(JSQMessagesInputToolbar *)toolbar
      didPressAudioBarButton:(UIButton *)sender
{
    if (toolbar.sendButtonOnRight) {
        [self didPressAudioButton:sender];
    }
    else {
        [self didPressSendButton:sender
                 withMessageText:[self jsq_currentlyComposedMessageText]
...
- (void)messagesInputToolbar:(JSQMessagesInputToolbar *)toolbar
 didPressExpressionBarButton:(UIButton *)sender{
    if (toolbar.sendButtonOnRight) {
        [self didPressExpressionButton:sender];
    }
    else {
        [self didPressSendButton:sender
                 withMessageText:[self jsq_currentlyComposedMessageText]
...
- (void)messagesInputToolbar:(JSQMessagesInputToolbar *)toolbar
  didPressDirectiveBarButton:(UIButton *)sender
{
    if (toolbar.sendButtonOnRight) {
        [self didPressDirectiveButton:sender];
    }
    else {
        [self didPressSendButton:sender
                 withMessageText:[self jsq_currentlyComposedMessageText]
...
- (void)messagesInputToolbar:(JSQMessagesInputToolbar *)toolbar
       didPressMoreBarButton:(UIButton *)sender
{
    if (toolbar.sendButtonOnRight) {
        [self didPressMoreButton:sender];
    }
    else {
        [self didPressSendButton:sender
                 withMessageText:[self jsq_currentlyComposedMessageText]
...
- (void)textViewDidBeginEditing:(UITextView *)textView
{
    ...
    //隐藏显示的扩展面板
    [self.inputToolbar textViewShowedBeginEditing];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if (context == kJSQMessagesKeyValueObservingContext) {
        if (object == self.inputToolbar.contentView.textView && [keyPath isEqualToString:NSStringFromSelector(@selector(contentSize))]) {
            ...
            //调整输入框高度
            _inputToolbar.contentView.textViewConstraint.constant += dy;
            
            //调整整体高度
            CGFloat height = self.inputToolbar.preferredDefaultHeight + dy;
            [self.inputToolbar setPreferredDefaultHeight:height];
        }
        if (object == self.inputToolbar && [keyPath isEqualToString:NSStringFromSelector(@selector(preferredDefaultHeight))]) {
            CGFloat oldContentSize = [[change objectForKey:NSKeyValueChangeOldKey] floatValue];
            CGFloat newContentSize = [[change objectForKey:NSKeyValueChangeNewKey] floatValue];
            CGFloat dy = newContentSize - oldContentSize;
...
- (BOOL)jsq_inputToolbarHasReachedMaximumHeight
{
    ...
    //+ (CGRectGetMaxY(self.inputToolbar.frame) - (self.topLayoutGuide.length + self.topContentAdditionalInset)) * 2 / 3;
}
...
- (void)jsq_addObservers
{
    ...
    [self.inputToolbar addObserver:self
                        forKeyPath:NSStringFromSelector(@selector(preferredDefaultHeight))
                           options:NSKeyValueObservingOptionOld | NSKeyValueObservingOptionNew
                           context:kJSQMessagesKeyValueObservingContext];
...
- (void)jsq_removeObservers
{
    ...
    @try {
        [_inputToolbar removeObserver:self
                                                forKeyPath:NSStringFromSelector(@selector(preferredDefaultHeight))
                                                   context:kJSQMessagesKeyValueObservingContext];
    }
    @catch (NSException * __unused exception) { }
...
-(void)jsq_movoTabbar{
    [self jsq_setToolbarBottomLayoutGuideConstant:300];
}
