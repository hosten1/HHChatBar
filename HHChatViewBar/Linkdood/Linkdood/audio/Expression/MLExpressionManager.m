...
- (NSDictionary*)expressionMapWithPlistName:(NSString*)plistName
{
    ...
    if (self.expressionMapRecords && [self.expressionMapRecords.allKeys count] > 0) {
        return self.expressionMapRecords;
    }
    //处理国际化问题
    ...
    NSMutableDictionary *emojiDic = [[NSMutableDictionary alloc]init];
    for (NSDictionary *dic in emojiArray) {
        [emojiDic setObject:[dic allValues][0] forKey:[dic allKeys][0]];
    }
    
    self.expressionMapRecords = emojiDic;
    return self.expressionMapRecords;
    /*
